...
export async function POST(request: NextRequest) {
  ...
  if (!process.env.OPENAI_API_KEY || process.env.OPENAI_API_KEY === 'your_openai_api_key_here') {
    // Возвращаем тестовый ответ вместо ошибки
    const body = await request.json();
    const { message } = body;
    
    const demoResponse = `Привет! Я Помогатор - твой AI-помощник. 

Вы написали: "${message}"

К сожалению, OpenAI API ключ не настроен, поэтому я не могу отвечать на вопросы. 

Для полноценной работы приложения необходимо:
1. Получить API ключ от OpenAI
2. Настроить переменную окружения OPENAI_API_KEY
3. Перезапустить приложение

Спасибо за понимание! 🚀`;

    return new Response(`data: {"content": "${demoResponse}"}\n\ndata: [DONE]\n\n`, {
      status: 200,
      headers: {
  ...
  try {
    ...
    // Эта проверка уже выполнена выше, убираем дублирование
